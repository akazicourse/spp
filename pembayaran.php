	...
	*/
	...
	if(isset($_REQUEST['submit'])){
		...
		if($submit=='hapus'){
			...
			$qbayar = mysqli_query($connect,"DELETE FROM pembayaran WHERE kelas='$jns' AND nis='$nis' AND bulan='$bln'");
...
<form class="form-inline" role="form" method="post" action="./admin.php?hlm=bayar">
  ...
  <div class="form-group">
      <label class="sr-only" for="jns">Jenis</label>
	  <select name="jns" class="form-control" id="jns">
	  <?php
		$qkelas = mysqli_query($connect, "SELECT jenis FROM jenis_bayar");
		while(list($jenis)=mysqli_fetch_array($qkelas)){
			echo '<option value="'.$jenis.'"></option>';
		}
	  ?>
	  </select>
  </div>
  ...
  <div class="form-group">
	...
	<div class="input-group">
		...
		<input type="text" class="form-control" id="jml" name="jml" placeholder="Jumlah" <?php echo $jumlah; ?>>
	</div>
  </div>
...
<?php
		...
		$qbayar = mysqli_query($connect,"SELECT kelas,bulan,tgl_bayar,jumlah FROM pembayaran WHERE nis='$nis' ORDER BY tgl_bayar DESC");
		if(mysqli_num_rows($qbayar) > 0){
			...
			while(list($jenis,$bulan,$tgl,$jumlah) = mysqli_fetch_array($qbayar)){
				...
				if( $_SESSION['admin'] == 1 ){
					echo '<a href="./admin.php?hlm=bayar&submit=hapus&kls='.$jenis.'&nis='.$nis.'&bln='.$bulan.'" class="btn btn-danger btn-xs">Hapus</a>';
				}
            echo ' <a href="./cetak.php?submit=nota&kls='.$jenis.'&nis='.$nis.'&bln='.$bulan.'" target="_blank" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-print" aria-hidden="true"></span></a>';
				echo '</td></tr>';
